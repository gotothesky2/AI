    ...
    try:
        ...
        full_data_query = text("""
            SELECT 
                ...
                a.cut_fifty as cut_fifty,
                a.cut_seventy as cut_seventy,
                a.cut_ninety as cut_ninety
            FROM admission_score a
            JOIN university u ON a.univ_id = u.univ_id
            JOIN major m ON a.major_id = m.m_id
            ORDER BY u.univ_name, m.m_name
        ...
        full_result = session.execute(full_data_query)
        full_data = []
        for row in full_result:
            full_data.append({
                '대학교명': row.university_name,
                '학과명': row.major_name,
                '전형명': row.admission_type,
                '50%컷': row.cut_fifty,
                '70%컷': row.cut_seventy,
                '90%컷': row.cut_ninety
            })
        
        # DataFrame 생성 및 CSV 저장
        df = pd.DataFrame(full_data)
        csv_filename = '../입학성적_전체데이터.csv'
        df.to_csv(csv_filename, index=False, encoding='utf-8-sig')
        print(f"전체 데이터를 '{csv_filename}' 파일로 저장했습니다.")
        print(f"저장된 파일 크기: {len(df)}행 x {len(df.columns)}열")
