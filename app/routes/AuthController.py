...
@router.post("/debug-token", summary="JWT 토큰 디버깅")
async def debug_jwt_token(token: str):
    """JWT 토큰을 디버깅하고 상세 정보를 반환합니다."""
    try:
        import jwt
        import os
        ...
        # provider_user_id로 사용자 조회 시도
        provider_user_id = payload.get("sub")
        user = None
        if provider_user_id:
            try:
                user = userRepository.getById(provider_user_id)
            except Exception as e:
                user_error = str(e)
        ...
        return create_success_response(
            {
                ...
                "user_lookup": {
                    ...
                    "user_error": user_error if 'user_error' in locals() else None
                }
            },
            "JWT 토큰 디버깅 완료"
        )
